...
- 标的可视化页面: http://127.0.0.1:8888/chart
...
在浏览器中直观查看某个标的的**进场期/退场期、场外指数、爆破指数**变化，并叠加**真实日 K 线**。
...
- **进场期/退场期**以背景色块（绿/红）贯穿三个面板
- **逼近日**以三角标记提示
- 顶部下拉选择标的（带 📈 标示有 K 线的标的）、关键词筛选
- X 轴联动缩放、十字光标

#### 数据实时性

- 页面数据**实时读取数据库**：运行 `./mag_system.sh <url>` 导入并分析后，**刷新页面即更新**，无需重新生成静态文件。
- **K 线惰性刷新**：页面加载时，若当天 K 线尚未更新，会在**后台**触发一次增量抓取并存库；一天最多一次，后续访问者直接读缓存的当日 K 线，不重复拉取。后台抓取不阻塞页面响应。

#### 数据源与覆盖

- 加密标的：Binance 现货日线（免 key）
- 美股 / 商品 / 亚股：Yahoo Finance（需未被限流的网络）
- 国内 A 股篮子（国内人工智能/机器人/生猪/地产等自定义标的）：无公开行情源，仅显示场外/爆破两栏

> K 线映射见 `src/kline_sources.py`；也可命令行手动抓取：`python3 src/fetch_kline.py [BTC ETH ...]`
...
返回 `/chart` 页面所需的结构化数据（实时读库），供页面 fetch 或二次开发使用。
...
```json
{
  ...
  "series": {
    "BTC": {
      ...
      "phase": ["进场期", "..."],
      "phase_days": [34, "..."],
      "ohlc": [[63479.99, 63490.86, 62802.27, 64010.0], null],
      "hasKline": true,
      "approaching": [12, 45],
      "segments": [{"phase": "进场期", "start": "2026-01-04", "end": "2026-01-08"}]
    }
  }
}
```
...
**字段说明**：
- `ohlc`：每项为 `[开盘, 收盘, 最低, 最高]`（ECharts 蜡烛图格式），无 K 线的日期为 `null`
- `hasKline`：该标的是否有真实 K 线（无则页面仅显示场外/爆破两栏）
- `segments`：进/退场期的连续区间（用于背景着色）

> 节点详情不内嵌在此接口中，由下面的 `/chart/nodes` 按标的懒加载。

---

### 5.1 节点详情接口

**端点**: `GET /chart/nodes?coin=BTC`

返回单个标的的关键节点与特殊节点，按日期归组，页面首次切到该标的时拉取并缓存（回测后会重新拉取，因为回测前会重算该标的节点）。

```json
{
  "2026-07-23": [
    {"kind": "key", "type": "break_200", "quality": "优质", "final_pct": 20.69,
     "ref_date": "2026-07-16", "ref_idx": 911.0, "cur_idx": 1145, "raw_pct": 25.69,
     "corr": {"相变": 5.0, "背离": -10.0}}
  ],
  "2026-08-13": [
    {"kind": "special", "type": "offchain_below_1000",
     "desc": "进场期场外指数跌破1000 - 场外指数：975，爆破指数：71"}
  ]
}
```
...
### v1.1.0 (2026-08-14)
- 新增 `GET /chart` 标的可视化页面（进/退场期 + 场外/爆破 + 真实日 K 线）
- 新增 `GET /chart/data` 实时数据接口
- K 线惰性刷新：页面加载时后台增量抓取并缓存，一天最多一次
